...
class template {
	...
	function complie() {
		...
		$template = preg_replace_callback("/\{\{eval (.*?)\}\}/is", array($this, 'complie_callback_stripvtag_1a'), $template);
		$template = preg_replace_callback("/\{eval (.*?)\}/is", array($this, 'complie_callback_stripvtag_1a'), $template);
		$template = preg_replace_callback("/\{for (.*?)\}/is", array($this, 'complie_callback_stripvtag_1b'), $template);

		$template = preg_replace_callback("/\{elseif\s+(.+?)\}/is", array($this, 'complie_callback_stripvtag_1c'), $template);
		...
		for($i=0; $i<2; $i++) {
			...
			$template = preg_replace_callback("/\{loop\s+$this->vtag_regexp\s+$this->vtag_regexp\}(.+?)\{\/loop\}/is", array($this, 'complie_callback_loopsection_123a'), $template);
		}
		$template = preg_replace_callback("/\{if\s+(.+?)\}/is", array($this, 'complie_callback_stripvtag_1d'), $template);

		$template = preg_replace("/\{template\s+(\w+?)\}/is", "<? include \$this->gettpl('\\1');?>", $template);
		$template = preg_replace_callback("/\{template\s+(.+?)\}/is", array($this, 'complie_callback_stripvtag_1e'), $template);
	...
	function complie_callback_stripvtag_1a($matches) {
		return $this->stripvtag('<? '.$matches[1].'?>');
	}

	function complie_callback_stripvtag_1b($matches) {
		return $this->stripvtag('<? for('.$matches[1].') {?>');
	}

	function complie_callback_stripvtag_1c($matches) {
	...
	function complie_callback_loopsection_123a($matches) {
		return $this->loopsection($matches[1], '', $matches[2], $matches[3]);
	}

	function complie_callback_stripvtag_1d($matches) {
		return $this->stripvtag('<? if('.$matches[1].') { ?>');
	}

	function complie_callback_stripvtag_1e($matches) {
	...
	function __destruct() {
		...
		$content = preg_replace_callback("/\<a(\s*[^\>]+\s*)href\=([\"|\']?)([^\"\'\s]+)/is", array($this, 'destruct_callback_transsid_312a'), ob_get_contents());
		$content = preg_replace("/(\<form.+?\>)/is", "\\1\n<input type=\"hidden\" name=\"sid\" value=\"".rawurldecode(rawurldecode(rawurldecode($sid)))."\" />", $content);
		ob_end_clean();
		echo $content;
	}

	function destruct_callback_transsid_312a($matches) {
