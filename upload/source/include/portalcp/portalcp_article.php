...
function portalcp_get_postmessage($post, $getauthorall = '') {
	...
	if(strpos($msglower, '[/audio]') !== FALSE) {
		$post['message'] = preg_replace_callback("/\[audio(=1)*\]\s*([^\[\<\r\n]+?)\s*\[\/audio\]/is", 'portalcp_get_postmessage_callback_parsearticlemedia_2a', $post['message']);
	}
	if(strpos($msglower, '[/flash]') !== FALSE) {
		$post['message'] = preg_replace_callback("/\[flash(=(\d+),(\d+))?\]\s*([^\[\<\r\n]+?)\s*\[\/flash\]/is", 'portalcp_get_postmessage_callback_parsearticlemedia_4a', $post['message']);
	}
...
function portalcp_get_postmessage_callback_parsearticlemedia_2a($matches) {
	return parsearticlemedia('mid,0,0', $matches[2]);
}

function portalcp_get_postmessage_callback_parsearticlemedia_4a($matches) {
