...
function followcode($message, $tid = 0, $pid = 0, $length = 0, $allowimg = true) {
	...
	if((strpos($message, '[/code]') || strpos($message, '[/CODE]')) !== FALSE) {
		$message = preg_replace_callback("/\s?\[code\](.+?)\[\/code\]\s?/is", "", $message);
	}
	...
	if(strpos($msglower, 'attach://') !== FALSE) {
		$message = preg_replace_callback("/attach:\/\/(\d+)\.?(\w*)/i", '', $message);
	}

	if(strpos($msglower, 'ed2k://') !== FALSE) {
		$message = preg_replace_callback("/ed2k:\/\/(.+?)\//", '', $message);
	}
	...
	if(strpos($msglower, '[/hide]') !== FALSE) {
		...
		if(strpos($msglower, '[hide=') !== FALSE) {
			$message = preg_replace_callback("/\[hide=(d\d+)?[,]?(\d+)?\]\s*(.*?)\s*\[\/hide\]/is", '', $message);
		}
	}
	...
	if($parsetype != 1 && strpos($msglower, '[swf]') !== FALSE) {
		$message = preg_replace_callback("/\[swf\]\s*([^\[\<\r\n]+?)\s*\[\/swf\]/is", 'followcode_callback_bbcodeurl_1a', $message);
	}
	$flag = $length ? 1 : 0;
	if($tid) {
	...
	if(!empty($_GET['highlight'])) {
		...
		followcode_callback_highlightword_21a($highlightarray, 1);
		$message = preg_replace("/<highlight>(.*)<\/highlight>/siU", "<strong><font color=\"#FF0000\">\\1</font></strong>", $message);
		$message = preg_replace_callback("/(^|>)([^<]+)(?=<|$)/sU", 'followcode_callback_highlightword_21a', $message);
		if($sppos !== FALSE) {
...
function fparsetable($width, $bgcolor, $message) {
	...
	if(strpos($message, '[/tr]') === FALSE && strpos($message, '[/td]') === FALSE) {
		...
		$message = preg_replace_callback("/\[tr(?:=([\(\)\s%,#\w]+))?\]\s*\[td(?:=(\d{1,4}%?))?\]/i", 'followcode_callback_parsetrtd_12a', $message);
		$message = preg_replace_callback("/\[\/td\]\s*\[td(?:=(\d{1,4}%?))?\]/i", 'followcode_callback_parsetrtd_1a', $message);
		$message = preg_replace_callback("/\[tr(?:=([\(\)\s%,#\w]+))?\]\s*\[td(?:=(\d{1,2}),(\d{1,2})(?:,(\d{1,4}%?))?)?\]/i", 'followcode_callback_parsetrtd_1234', $message);
		$message = preg_replace_callback("/\[\/td\]\s*\[td(?:=(\d{1,2}),(\d{1,2})(?:,(\d{1,4}%?))?)?\]/i", 'followcode_callback_parsetrtd_123a', $message);
		$html = '<table cellspacing="0" class="t_table" '.
			($width == '' ? NULL : 'style="width:'.$width.'"').
			($bgcolor ? ' bgcolor="'.$bgcolor.'">' : '>').
...
function followcode_callback_bbcodeurl_1a($matches) {
...
function followcode_callback_highlightword_21a($matches, $action = 0) {
...
function followcode_callback_parsetrtd_12a($matches) {
	return parsetrtd($matches[1], 0, 0, $matches[2]);
}

function followcode_callback_parsetrtd_1a($matches) {
	return parsetrtd('td', 0, 0, $matches[1]);
}

function followcode_callback_parsetrtd_1234($matches) {
	return parsetrtd($matches[1], $matches[2], $matches[3], $matches[4]);
}

function followcode_callback_parsetrtd_123a($matches) {
