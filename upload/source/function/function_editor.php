...
function html2bbcode($text) {
	...
	$text = preg_replace_callback('/<h([0-9]+)[^>]*>/siU', 'html2bbcode_callback_1a', $text);
...
function html2bbcode_callback_1a($matches) {
...
function parsestyle($tagoptions, &$prependtags, &$appendtags) {
	...
	$style = preg_replace_callback("/(?<![a-z0-9-])color:\s*rgb\((\d+),\s*(\d+),\s*(\d+)\)(;?)/i", 'parsestyle_callback_sprintf_4123a', $style);
...
function parsestyle_callback_sprintf_4123a($matches) {
